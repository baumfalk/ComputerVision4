...
namespace nl_uu_science_gmt
{
	...
	Detector::Detector(const std::string &qif) :
		_pos_amount(Detector::cfg()->getValue<int>("settings/images@amount")),
		_target_width(Detector::cfg()->getValue<int>("settings/images@width")), _posneg_factor(
	...
	*/
	void Detector::readPosData(const std::vector<std::string> &pos_train, cv::Mat &pos_data, cv::Mat &pos_data_hog)
	{
		...
		for (size_t f = 0; f < pos_train.size(); ++f)
		{
			...
			Mat hogMat(1, featurevector.size(), CV_32F);
			for (size_t i = 0; i < featurevector.size(); i++)
			{
				hogMat.at<float>(0, i) = featurevector[i];
			}
			pos_data_hog.push_back(hogMat);
		...
		cout << pos_data.rows << endl;
		cout << pos_data_hog.rows << endl;
	}

	/*
	* Read negative image data
	*/
	void Detector::readNegData(const std::vector<std::string> &neg_train, cv::Mat &neg_data, cv::Mat &neg_data_hog)
	{
	...
	*/
	...
	void Detector::run()
	{
		...
		vector<string> pos_val(pos_files.begin() + pos_amount,
			pos_files.begin() + MIN(2 * pos_amount, (int)pos_files.size() - pos_amount));
		...
		Mat pos_train_data, neg_train_data, pos_train_data_hog, neg_train_data_hog;
		cout << endl << "line:" << __LINE__ << ") Read training images" << endl;
		cout << "==============================" << endl;

		readPosData(pos_train, pos_train_data, pos_train_data_hog);
		readNegData(neg_train, neg_train_data, neg_train_data_hog);
		...
		Mat train_data_hog;
		train_data_hog.push_back(pos_train_data_hog);
		train_data_hog.push_back(neg_train_data_hog);
		...
		cout << "==============================" << endl;
		Mat pos_val_data, neg_val_data, pos_val_data_hog, neg_val_data_hog;
		readPosData(pos_val, pos_val_data, pos_val_data_hog);
		readNegData(neg_val, neg_val_data, neg_val_data_hog);
		...
		Mat val_data_hog;
		val_data_hog.push_back(pos_val_data_hog);
		val_data_hog.push_back(neg_val_data_hog);
		...
		cout << "train_data cols" << train_data.cols << endl;
		cout << "train_data rows" << train_data_hog.rows << endl;
		cout << "train_data cols" << train_data_hog.cols << endl;
		cout << "Labels     rows" << labels.rows << endl;
		cout << "Labels     cols" << labels.cols << endl;
	
		// Train the SVM
		cout << "line:" << __LINE__ << ") Training SVM..." << endl;
		svm.train(train_data_hog, labels, Mat(), Mat(), params);
	
		Mat labels_train;
		svm.predict(train_data_hog, labels_train);
...

		{
			...
			Mat conf_train = (train_data_hog * WTrans_32F) + b;
			Mat conf_val = (val_data_hog * WTrans_32F) + b;
		...
		Mat W_rect(height, width, CV_32F);
		...
		// outcommented because else crash.
		//imshow("Model", W_rect);
		cout << "Press a key to continue" << endl;
		waitKey();
		/////////////////////////////////////////////////////////////////////////////
		...
		for (size_t layer = 0; layer < pyramid.size(); ++layer)
		{
			...
			for (size_t i = 0; i < X1.total(); ++i)
			{
				...
				Mat sub1d = hogMat;
				Mat sub1dF;
				sub1d.convertTo(sub1dF, CV_32F);
				
				Mat mean, stddev;
				meanStdDev(sub1dF, mean, stddev);
				Mat mean_line(1, sub1dF.cols, CV_32F);
				mean_line = mean.at<double>(0, 0);
				Mat std_line(1, sub1dF.cols, CV_32F);
			...
			*/
			...
			Mat detect = (svm.predict(sub_windows, true) * WTrans_32F) + b;*/
			...
			if (o_value != o_val)
			{
				...
				*/
				...
				if (!ground_truths.empty())
				{
					...
					vector<bool>facefound(search_gt.size());
					for (size_t r = 0; r < rects.size(); ++r)
					{
						...
						for (int i=0; it != search_gt.end();i++)
						{
					...
					for (size_t i = 0; i < facefound.size(); i++)
					{
						if (facefound[i] == false) {
							falseNegatives++;
						}
					}
					cout << truePositives << endl;
					cout << falsePositives << endl;
					cout << falseNegatives << endl;
